...
def parse_inventory(raw: str | bytes | dict) -> Inventory:
    ...
    if isinstance(raw, (str, bytes)):
        if len(raw) > MAX_INVENTORY_BYTES:
            raise DiscoveryError("inventory too large")
        if isinstance(raw, bytes):
            raw = raw.decode("utf-8-sig", errors="replace")
        try:
            raw = json.loads(raw.lstrip("﻿"))
        except json.JSONDecodeError as e:
            raise DiscoveryError(f"collector output is not valid JSON: {e}") from e
    try:
        return Inventory.model_validate(raw)
    except ValidationError as e:
        raise DiscoveryError(f"inventory failed schema validation: {e.errors()[:5]}") from e
